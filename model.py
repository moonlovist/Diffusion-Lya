...
class ResidualBlock1D(nn.Module):
    ...
    def forward(self, x, t_emb):
        ...
        h = self.norm1(h)
        h = self.act(h)
        t_add = self.time_proj(t_emb)[:, :, None]
        h = h + t_add
        h = self.conv2(h)
        h = self.norm2(h)
        h = self.act(h)
...
class UNet1D(nn.Module):
    def __init__(self, in_ch=1, base_ch=64, depth=4, time_dim=256, groups=8):
        ...
        in_c = base_ch
        for out_c in chs:
            self.down_blocks.append(ResidualBlock1D(in_c, out_c, time_dim, groups=groups))
            in_c = out_c
        for _ in range(depth - 1):
            self.downsamples.append(nn.Conv1d(in_c, in_c, kernel_size=4, stride=2, padding=1))

        self.mid_block1 = ResidualBlock1D(in_c, in_c, time_dim, groups=groups)
        self.mid_block2 = ResidualBlock1D(in_c, in_c, time_dim, groups=groups)
        ...
        for out_c in reversed(chs[:-1]):
            ...
            self.up_blocks.append(ResidualBlock1D(out_c * 2, out_c, time_dim, groups=groups))
            in_c = out_c
